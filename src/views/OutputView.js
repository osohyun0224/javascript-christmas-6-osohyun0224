...
const OutputView = {
  ...
  printEventPreview(visitDate) {
    Console.print(
      `${GUIDE_MESSAGE.startEventMonth}${visitDate}${GUIDE_MESSAGE.startEventDay}${GUIDE_MESSAGE.startEventGuide}\n`,
    );
  },
  ...
  printOrderMenu(orderItems) {
    ...
    Console.print(
      orderItems
        .map((item) => `${item[0]} ${item[1]}${GUIDE_MESSAGE.unitCount}`)
        .join("\n") + "\n",
    );
  },
  ...
  printBenefitDetails(discountsApplied) {
    ...
    if (discountsApplied.length > 0) {
      const formattedDiscounts = discountsApplied.map((discount) => {
        ...
        return `${discount.replace(/-?\d+/, formatCurrency(discountAmount))}${
          GUIDE_MESSAGE.unitMoney
        }`;
      });
  ...
  printTotalBenefitAmount(totalDiscount) {
    ...
    const amountString =
      totalDiscount > 0
        ? `${NEGATIVE_SIGN}${formatCurrency(totalDiscount)}`
        : `${ZERO_AMOUNT}`;
